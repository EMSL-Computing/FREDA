...
equations <- list("$\\qquad\\frac{O}{C}$", "$\\qquad\\frac{H}{C}$", "$\\qquad\\frac{N}{C}$", "$\\qquad\\frac{P}{C}$", "$\\qquad\\frac{N}{P}$",
                  ...
                  "$\\qquad 1 + C - O - S - 0.5*(N + P + H)$", "$\\qquad 1 + C - O - S - 0.5*(N + P + H) - O$", "$\\qquad$Combination of C,H,N,O,S,P")
```
...
dontoutput <-lapply(6:length(equations), function(i){
  if(calc_cols$ColumnName[i] %in% proc_cols){
    pander(paste(cols_for_display[which(cols_for_display == calc_cols$DisplayName[i])], " | "))
    pander(equations[[i]])
